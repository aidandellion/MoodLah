...
function initMoodHistory() {
    ...
    function renderTable() {
        ...
        dates.forEach(date => {
            ...
            const theme = styleMap[entry.mood] || { class: "secondary", icon: "ri-question-mark" };

            row.innerHTML = `
                <td>${formatDate(date)}</td>
                <td>
                    <span class="badge bg-${theme.class}-subtle text-${theme.class} fs-12">
                        <i class="${theme.icon} align-middle me-1"></i> ${entry.mood}
                    </span>
                </td>
                <td class="text-truncate" style="max-width: 250px;">${entry.note || "-"}</td>
                <td>
